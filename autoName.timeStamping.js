...
function addStamping(e)
{
  ...
  else if(col === targetCol2 && row > startRow && e.source.getActiveSheet().getName() === wS_Tab2)
  {
    ...
    if(e.source.getActiveSheet().getRange(row,17).getValue() == "") //endif date created
    {
      ...
      // var protection1 = e.source.getActiveSheet().getRange(row,17).protect();
      // e.source.getActiveSheet().getRange(row,16).protect(protection);

      // protection.removeEditors(protection.getEditors());
      // protection.setDomainEdit(false);
      // protection1.removeEditors(protection.getEditors());
      // protection1.setDomainEdit(false);

      /* if (protection.canDomainEdit() || protection1.canDomainEdit())
      {
        protection.setDomainEdit(false);
        protection1.setDomainEdit(false);
      } */
    ...
    /* if(e.source.getActiveSheet().getRange(row,16).getValue() != "")
    {
      let protection = e.source.getActiveSheet().getRange(row,16).protect();
      protection.removeEditors(protection.getEditors());
      if (protection1.canDomainEdit())
        {
          protection1.setDomainEdit(false);
